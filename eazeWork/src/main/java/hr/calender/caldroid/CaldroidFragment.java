...
import hr.calender.infiniteviewpager.InfinitePagerAdapter;
import hr.calender.infiniteviewpager.InfiniteViewPager;
...
public class CaldroidFragment extends DialogFragment {
	...
	public final static String DIALOG_TITLE = "dialogTitle", MONTH = "month",
			...
			LEAVE_DATES = "leaveDates", HALF_DAY_DATES = "halfDayDates",
			COMP_OFF_DATES = "compOffDates",
	...
	protected ArrayList<DateTime> compOffDates = new ArrayList<DateTime>();
	protected ArrayList<DateTime> halfDayDates = new ArrayList<DateTime>();
	...
	public HashMap<String, Object> getCaldroidData() {
		...
		caldroidData.put(HALF_DAY_DATES, halfDayDates);
	...
	public Bundle getSavedStates() {
		...
		if (halfDayDates != null && halfDayDates.size() > 0) {
			bundle.putStringArrayList(HALF_DAY_DATES,
					CalendarHelper.convertToStringList(halfDayDates));
		}
	...
	public void moveToDateTime(DateTime dateTime) {
		...
		if (dateTime.lt(firstOfMonth)) {
			...
			// decrease
			DateTime firstDayNextMonth = dateTime.plus(0, 1, 0, 0, 0, 0,
		...
		// Calendar swipe right when dateTime is in the future
			...
			// increase
			DateTime firstDayLastMonth = dateTime.minus(0, 1, 0, 0, 0, 0,
					DateTime.DayOverflow.LastDay);
	...
	/**
	 * To clear leaveDates. This method does not refresh view, need to
	 * explicitly call refreshView()
	 */
	...
	/**
	 * Set Date as Leave Date of Employee By default the background color is
	 * holo_red_light, and the text color is white. You can customize the
	 * background by changing CaldroidFragment.selectedBackgroundDrawable, and
	 * change the text color CaldroidFragment.selectedTextColor before call this
	 * method. This method does not refresh view, need to call refreshView()
	 * 
	 * @param leaveList is list of Leave Dates
	 */
	public void setLeaveDates(ArrayList<DateTime> leaveList) {
		// Ensure fromDate is before toDate
		if (leaveList == null) {
			return;
		}
		...
		leaveDates.clear();
		for (DateTime date : leaveList) {
			//DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
			leaveDates.add(date);
		}
	}

	public void setCompOffDates(ArrayList<DateTime> leaveList) {
		// Ensure fromDate is before toDate
		if (leaveList == null) {
			return;
		}
		...
		for (DateTime date : leaveList) {
			//DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
			compOffDates.add(date);
		}
	}
	public void setLeaveIntervalDates(Date fromDate, Date toDate) {
		// Ensure fromDate is before toDate
		if (fromDate == null || toDate == null || fromDate.after(toDate)) {
			return;
		}
...
/**
 * Set date to mark as Leave Day
 * @param halfDay
 */
	public void addLeaveDayDate(Date leaveDay) {
		// Ensure fromDate is before toDate
		if (leaveDay == null) {
			return;
		}
			DateTime dateTime = CalendarHelper.convertDateToDateTime(leaveDay);
			leaveDates.add(dateTime);
	}
	/**
	 * To clear halfDayLeaves. This method does not refresh view, need to
	 * explicitly call refreshView()
	 */
	public void clearHalfDayDates() {
		halfDayDates.clear();
	}
	 ...
	 * @param toDate
	 */
	public void setHalfDayDates(ArrayList<DateTime> halfDayList) {
		// Ensure fromDate is before toDate
		if (halfDayList == null) {
			return;
		}
		...
		halfDayDates.clear();
		for (DateTime date : halfDayList) {
			//DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
			halfDayDates.add(date);
		}
	}
/**
 * Add date to mark as Half Day
 * @param halfDay
 */
	public void addHalfDayDates(Date halfDay) {
		// Ensure fromDate is before toDate
		if (halfDay == null) {
			return;
		}
			DateTime dateTime = CalendarHelper.convertDateToDateTime(halfDay);
	...
	public int getCurrentMonth(){
		return firstMonthTime.month;
	}
	public int getCurrentYear(){
		return firstMonthTime.year;
	}
	/**
		...
		public void refreshAdapters(int position) {
			...
			if (position == currentPage) {
				...
				nextAdapter.setAdapterDateTime(currentDateTime.plus(0, 1, 0, 0,
						0, 0, DateTime.DayOverflow.LastDay));
			...
			// Swipe right
				...
				currentDateTime = currentDateTime.plus(0, 1, 0, 0, 0, 0,
						DateTime.DayOverflow.LastDay);
			...
			// Swipe left
				...
				currentDateTime = currentDateTime.minus(0, 1, 0, 0, 0, 0,
						DateTime.DayOverflow.LastDay);
	...
	public void notifyDataSetChanged() {
		pageChangeListener.refreshAdapters(pageChangeListener.getCurrentPage());//.notifyDataSetChanged();
	}
}
